(function ($) {
    ...
    // Back to top button (scroll - header)
    
    $(window).scroll(function () {
        if ($(this).scrollTop() > 200) {
            $('.back-to-top').fadeIn('slow');
            let logo= document.getElementById("logo");
            logo.src="img/LOGO_BLANCO_MOVILNET.png";
            logo.style.width="200px";
            
        } else {
            $('.back-to-top').fadeOut('slow');
            let logo= document.getElementById("logo");
            logo.src="img/LOGO_MOVILNET_BC.png";
...
//JS Lista de imagenes Clientes (Carousel)
...
//JS Lista de imagenes PROYECTOS (Carousel)
window.addEventListener('load', function(){
	new Glider(document.querySelector('.carousel__listaP'), {
		slidesToShow: 1,
		slidesToScroll: 1,
		dots: '.carousel__indicadoresP',
		arrows: {
			prev: '.carousel__anteriorP',
			next: '.carousel__siguienteP'
		},
		responsive: [
			{
			  // screens greater than >= 450px
...
/*******ACTIVE SOLUCIONES ITEM******************/
$('[data-spy="scroll"]').on('activate.bs.scrollspy', function () {
    // do something...
  })

  /////************************PROVINCIAS LISTA DE DEPARTAMENTO, PROVINCIAS Y DISTRITOS************************//
  function cambia()
{
